...
public class King implements Piece{
    ...
    public boolean canMove(Board a, String h){
        h=h.toLowerCase();// add these lines to all pieces
        ...
        if(compLet.equals("a")){//add whole if else block to other pieces
            moveCol = 0;
        }
        ...
        //dont need to check in between, king can only move 1 space
        //line below makes sure space entered was valid
        if(moveRow!=-1&&moveCol!=-1){
            if((Math.abs(rowOn-moveRow)==1&&moveCol==colOn)||(Math.abs(colOn-moveCol)==1&&moveRow==rowOn)||(Math.abs(colOn-moveCol)==1&&Math.abs(rowOn-moveRow)==1)){
                //makes sure move to space is in one of surrounding boardSpaces
