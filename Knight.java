...
public class Knight implements Piece{
    ...
    public boolean canMove(Board a, String h){
        ...
        if(moveRow!=-1&&moveCol!=-1){
            if((Math.abs(colOn-moveCol)==2&&Math.abs(rowOn-moveRow)==1)||(Math.abs(colOn-moveCol)==1&&Math.abs(rowOn-moveRow)==2)){
                //makes sure move spot is valid for Knight^
                if(arr[moveRow][moveCol].isEmpty()){
                    ...
                    if(this.getColor()!=compareTo){
                        System.out.println("PIECE BEING TAKEN BY KNIGHT: "+g.get(removeInd));
