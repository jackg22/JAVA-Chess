...
public class Queen implements Piece{
    ...
    public boolean canMove(Board a, String h){
        ...
        if(moveRow!=-1&&moveCol!=-1){
            // Horizontal/Vertical Movements Not taking piece(Copied from Rook)
            if(moveCol==colOn||moveRow==rowOn){
                if(arr[moveRow][moveCol].isEmpty()){
                    ...
                    if(moveRow==rowOn){
                        ...
                        if(moveCol<colOn){
                            ...
                            for(int i = moveCol+1;i<colOn;i++){
                                System.out.println("queen CANT move");//
                    ...
                    //MOVING HORIZONTALLY/VERTICALLY TAKING PIECE
            ...
            //END OF ANY VERTICAL/HORIZONTAL SHIFTS
            if(Math.abs(colOn-moveCol)==Math.abs(rowOn-moveRow)){
                if(arr[moveRow][moveCol].isEmpty()){
                    boolean checkInBetween = true;
                    if(moveRow<rowOn&&moveCol<colOn){
                        //space is up left of piece
                        for(int i = 1;i<=Math.abs(moveCol-colOn);i++){
                            if(!(arr[rowOn-i][colOn-i].isEmpty())){
                                checkInBetween = false;
                            }
                        }
                    ...
                    if(this.getColor()!=compareTo){
                        //code here for if the moveTo space is taken by a opposite piece
                        boolean checkInBetween = true;
                        if(moveRow<rowOn&&moveCol<colOn){
                            //space is up left of piece
                            for(int i = 1;i<=Math.abs(moveCol-colOn);i++){
