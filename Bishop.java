...
public class Bishop implements Piece{
    ...
    public boolean canMove(Board a, String h){
        ...
        if(moveRow!=-1&&moveCol!=-1){       //as long as difference in x/y values is the same, bishop can move to that boardSpace
            if(Math.abs(colOn-moveCol)==Math.abs(rowOn-moveRow)){
                if(arr[moveRow][moveCol].isEmpty()){
                    ...
                    if(this.getColor()!=compareTo){
                        //code here for if the moveTo space is taken by an opposite piece
                        boolean checkInBetween = true;
                        if(moveRow<rowOn&&moveCol<colOn){
                            //space is up left of piece
                            for(int i = 1;i<=Math.abs(moveCol-colOn);i++){
